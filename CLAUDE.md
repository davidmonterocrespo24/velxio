...
- **`PartSerialAPI.write` + `PartI2CAPI.registerSlave` — closing the write side of the high-level API (SDK-003b-step4)** — Cierra la mitad write del `PartSimulationAPI` que SDK-003b-step2 dejó documentada como "reserved for a future ticket". Tres adiciones contractuales en `packages/sdk/src/simulation.ts`: (1) `SimulatorHandle.injectSerialRx(data: Uint8Array | string): void` como primitiva low-level; (2) `PartSerialAPI.write(data: Uint8Array | string): void` que envuelve `injectSerialRx` con permission gate; (3) `PartI2CAPI.registerSlave(addr: number, handler: I2cSlaveHandler): Disposable` que envuelve el `SimulatorHandle.registerI2cSlave` ya existente para que un `HighLevelPartSimulation` pueda actuar como esclavo I²C sin tener que capturar el handle a mano. **Permission gates asimétricos** (call-time, NO register-time): registrar el part sigue requiriendo solo `simulator.pins.read`; `serial.write` añade nuevo permiso **`simulator.serial.write`** (High risk — agregado al `PLUGIN_PERMISSIONS` enum + `permissions-catalog.ts` + fila en `docs/PLUGIN_PERMISSIONS.md`, sync test catalog↔docs verde); `i2c.registerSlave` reutiliza el `simulator.i2c.write` existente (la fila del catálogo se actualizó para mencionar el nuevo entry point). Host adapter `frontend/src/simulation/parts/PartSimulationRegistry.ts` `injectSerialRx` hace **per-board dispatch**: prefiere `serialWriteByte(byte)` byte-a-byte si está disponible (RP2040 path, evita el round-trip a string), si no `serialWrite(text)` con `TextDecoder('latin1').decode(data)` para bytes→string (AVR/ESP32/RISC-V — **1:1 byte-preserving**: UTF-8 expandiría bytes ≥ 0x80 silenciosamente; `String.fromCharCode(...data)` también funcionaría pero trips el límite de ~65K args de V8 en payloads grandes), si tampoco silent no-op (board sin UART o todavía no arrancada). String→bytes via `Uint8Array.from(s, ch => ch.charCodeAt(0) & 0xff)` — UART es byte-stream, **NO text-stream**. `buildPartSimulationAPI` en `createPluginContext.ts` trackea ambas suscripciones en el array `disposables[]` con LIFO unwind: la `i2c.registerSlave` Disposable se libera aún cuando el autor olvide llamar `dispose()` explícitamente. Tests: 5 SDK type tests añadidos a `packages/sdk/test/high-level-part.test.ts` (signature pins de `serial.write`/`i2c.registerSlave`/`SimulatorHandle.injectSerialRx`, `PLUGIN_PERMISSIONS` enum membership, `PluginPermission` literal narrowing) + 10 host integration tests en `frontend/src/__tests__/plugin-host-sdk003b-step4.test.ts` cubriendo los 7 casos de aceptación más 3 edge variants: (a) `serial.write` sin permiso lanza `PermissionDeniedError` y NO llama simulador; (b) `serial.write(string)` AVR path forwarda a `simulator.serialWrite('AT\r\n')`; (c) `serial.write(Uint8Array)` AVR path con bytes 0x48/0x69/0xff round-trips perfecto vía latin1 (cada `charCodeAt` matches el byte input); (d) RP2040 path con bytes prefiere byte-a-byte (3 calls a `serialWriteByte`, fallback `serialWrite` no se invoca); (e) RP2040 path con string convierte vía charCodeAt y dispatch byte-a-byte; (f) silent no-op sin `serialWrite`/`serialWriteByte`; (g) `i2c.registerSlave` sin permiso lanza y nunca toca el bus; (h) install→`bus.addDevice` + `dispose()`→`bus.removeDevice`; (i) teardown del API libera slaves olvidados (2 slaves, 0 disposes manuales, ambos eliminados); (j) silent no-op sin `i2cBus`. 287/287 SDK + 132/132 plugin-host sweep verde. `docs/PLUGIN_SDK.md` extiende la tabla del PartSimulationAPI surface con las nuevas filas + nueva subsección "Injecting bytes into the MCU UART RX and acting as an I²C slave" con ejemplos completos `gpsModem` (NMEA stream injection vía `setInterval`) y `ds1307Rtc` (clock chip respondiendo a master I²C reads desde un buffer interno).
