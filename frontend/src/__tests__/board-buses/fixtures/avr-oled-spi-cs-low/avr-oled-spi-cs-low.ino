...
//
// Variant of avr-oled-spi: chip select is pulled LOW straight out of reset
// (pinMode OUTPUT, digitalWrite LOW, never HIGH first), the minimal driver a
// user writes for a lone panel. The pin goes from floating to driven LOW, so
// there is no HIGH-to-LOW edge for a part to see: only the pin's level says
...
//   --fqbn arduino:avr:uno --out <this dir> <this dir>/avr-oled-spi-cs-low.ino
#include <SPI.h>
...
void setup() {
  Serial.begin(115200);
  // Before SPI.begin(): D10 is the Uno's SS, and SPI.begin() drives an SS
  // that is still an input HIGH. Configured first, it is left alone.
